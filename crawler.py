...
MAX_FILE_BYTES = 200 * 1_024    # > 200 KB → too long for elementary chunking
...
NONFICTION_SUBJECTS = {
    ...
    "marine biology", "ecology", "wildlife", "plants",
    # Earth & space
    "astronomy", "geology", "geography", "volcanoes", "weather",
    # Human body & health
    "physiology", "anatomy", "medicine", "biology",
    # Inventors, science & technology
    "inventors", "inventions", "engineering", "electricity", "physics",
    "chemistry", "science", "mechanics",
    # Exploration & adventure non-fiction
    "exploration", "explorers", "travel", "voyages", "discovery",
    # History & civilizations
    "history", "ancient history", "ancient civilizations", "archaeology",
    "medieval", "civilization", "egypt", "greece", "rome",
    # Biography
...
REJECT_SUBJECTS = {
    ...
    "linguistics", "grammar", "government", "sociology",
}
...
SCREEN_SYSTEM = """You are a curriculum specialist evaluating public domain texts
for an AI tutor serving elementary school students (grades K-6, ages 5-12).
Assess whether a text is BOTH engaging for modern kids AND appropriate in
content and reading accessibility.

Respond ONLY with a JSON object - no markdown, no preamble. Schema:
{
  "score": <integer 1-5>,
  "approved": <true|false>,
  "topics": [<2-5 modern elementary subject tags, e.g. "animals", "space", "inventors">],
  "reasoning": "<one sentence explaining the score>"
}

Scoring rubric:
5 - Vivid, narrative, clearly relevant to a modern elementary topic, age-appropriate language
4 - Engaging and relevant, minor accessibility issues (slightly archaic vocabulary)
3 - Usable with adaptation; topic relevant but prose is dry or vocabulary challenging
2 - Topic mismatch or text too dense/abstract for elementary students
1 - Completely inappropriate (adult themes, impenetrable prose, wrong subject)

approved = true if score >= 3, false otherwise."""


def llm_screen(record: BookRecord, preview_text: str) -> BookRecord:
    if not ANTHROPIC_API_KEY:
        log.warning("[%d] No ANTHROPIC_API_KEY — skipping LLM screen, auto-approving",
                    record.gutenberg_id)
        record.llm_score    = 3
    ...
    try:
        ...
        record.llm_approved  = bool(result.get("approved", False))
        record.llm_topics    = result.get("topics", [])
...
def parse_catalog(raw_gz: bytes) -> list[BookRecord]:
    ...
    with gzip.open(io.BytesIO(raw_gz), "rt", encoding="utf-8") as f:
        ...
        for raw_row in reader:
            ...
            # Language
            if find_col(row, "language", "languages").strip().lower() != TARGET_LANGUAGE:
                continue

            # Type
            type_val = find_col(row, "type", "types").strip().lower()
            if type_val and type_val not in ("text", "dataset", ""):
                continue

            # Era filter
            issued_raw  = find_col(row, "issued", "date", "issued date")
            issued_year = parse_year(issued_raw)
            if issued_year is None or not (ISSUED_YEAR_MIN <= issued_year <= ISSUED_YEAR_MAX):
                continue
    ...
    log.info("Scanned %d rows → %d candidates (era %d-%d, elementary subjects)",
             total, len(records), ISSUED_YEAR_MIN, ISSUED_YEAR_MAX)
    return records
...
def main():
    ...
    if not ANTHROPIC_API_KEY:
        log.warning("ANTHROPIC_API_KEY not set — LLM screening disabled, "
